...
elif [ "$Box_Location" == "KN" ]; then
  BOX_HOST=$(get_config_value configuration/init.ini box O_KN_IP)
  BOX_PASS=$(get_config_value configuration/init.ini box O_KN_Password)
else
...
sshpass -p $BOX_PASS ssh -o StrictHostKeyChecking=no root@$BOX_HOST "iptables -t nat -A POSTROUTING -o br-ex -s $network_pool.0/24 -j MASQUERADE && iptables -A FORWARD -i br-ex -d $network_pool.0/24 -m state --state RELATED,ESTABLISHED -j ACCEPT && iptables -A FORWARD -i br-ex -d $network_pool.0/24 -j ACCEPT"

sshpass -p $BOX_PASS ssh -o StrictHostKeyChecking=no root@$BOX_HOST "netfilter-persistent save && netfilter-persistent reload"

sshpass -p $BOX_PASS ssh -o StrictHostKeyChecking=no root@$BOX_HOST "ip addr add $network_pool.10/24 dev br-ex"

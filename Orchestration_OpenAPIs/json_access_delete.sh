...
if [ "$REGION" == "GJ" ]; then
	...
	# Type S - Type C edge
        ...
        # find Edge between new node (KN-C- new node)
        lineNum="$(grep -n "\"id\":\"$Slicing_ID-JJ_O-" api/slices/static/test.json | head -n 1 | cut -d: -f1)"
...
elif [ "$REGION" == "JNU" ]; then
        # Type S - Type C edge

        # find Edge between new node (KN-C- new node)
        lineNum="$(grep -n "\"id\":\"$Slicing_ID-JNU_O-" api/slices/static/test.json | head -n 1 | cut -d: -f1)"
        ...
        # find Edge between new node (KN-C- new node)
        lineNum="$(grep -n "\"id\":\"$Slicing_ID-KU_O-" api/slices/static/test.json | head -n 1 | cut -d: -f1)"
